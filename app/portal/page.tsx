...
export default function DashboardPage() {
  const { studentData } = usePortalAuth(); const data = usePortalData();
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-2xl border bg-white p-6 shadow-sm"><h3 className="flex items-center gap-2 font-black"><CalendarClock className="text-[#6C3CE9]" />Upcoming classes</h3><div className="mt-4 grid gap-3 md:grid-cols-2">{data.upcomingClasses.slice(0, 4).map((item) => <div key={item.id} className="rounded-xl border border-slate-200 p-4"><p className="font-bold">{item.title}</p><p className="mt-1 text-sm text-slate-500">{item.date} · {item.time}</p></div>)}{!data.upcomingClasses.length && <p className="text-sm text-slate-500">No upcoming classes scheduled.</p>}</div></section>
  </div>;
}
