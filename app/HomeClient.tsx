...
export default function HomeClient({ initialCourses = [] }: { initialCourses?: PublicCourse[] }) {
  const { displayCourses, courses, loading: coursesLoading } = usePublicCourses(initialCourses);
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-500/30 overflow-hidden">
      ...
      <section className="relative pt-32 pb-20 lg:pt-18 lg:pb-32 px-6">
        ...
        <div className="container mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <motion.div variants={fadeInUp} className="inline-block px-4 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-600 text-xs font-bold mb-6 tracking-wide uppercase backdrop-blur-sm">
            ...
            <motion.p variants={fadeInUp} className="text-lg text-slate-600 mb-8 max-w-lg leading-relaxed">
            ...
            <motion.div variants={fadeInUp} className="flex flex-wrap gap-4">
                ...
                <Link 
                  href="/contact" 
                  className="px-8 py-4 rounded-full border border-slate-300 hover:bg-slate-100 transition-all font-semibold flex items-center justify-center"
                >
                  Contact Us
                </Link>
            </motion.div>
          ...
          >
            <div className="w-full aspect-square bg-gradient-to-tr from-blue-600/10 to-cyan-400/10 rounded-[2.5rem] border border-white/20 backdrop-blur-xl flex items-center justify-center shadow-2xl overflow-hidden relative">
              <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=1000')] bg-cover bg-center opacity-10 mix-blend-overlay"></div>
              <div className="relative z-10 grid grid-cols-2 gap-6 p-8">
                 {[Code, MonitorPlay, Database, Award].map((Icon, i) => (
                    <div key={i} className="w-24 h-24 bg-white/80 backdrop-blur-xl border border-white/40 rounded-2xl flex items-center justify-center shadow-xl animate-bounce" style={{animationDelay: `${i * 0.2}s`, animationDuration: '3s'}}>
      ...
      {/* 2. Stats Section */}
      <section className="py-12 border-y border-slate-200 bg-white/50 backdrop-blur-md relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-slate-200">
            {stats.map((stat, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.1 }} className="text-center px-4">
                ...
                <p className="text-sm font-semibold text-slate-500 mt-2 uppercase tracking-wide">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-slate-100 relative border-y border-slate-200">
        <div className="container mx-auto max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="text-3xl md:text-5xl font-bold mb-4 text-slate-900">Popular <span className="text-blue-600">Courses</span></h2>
              <p className="text-slate-600 max-w-xl text-lg">Master the most in-demand skills of 2026. From basic computing to advanced AI tools.</p>
            </div>
            <Link href="/courses" className="flex items-center gap-2 text-blue-600 font-semibold hover:gap-3 transition-all">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            {!coursesLoading && displayCourses.length === 0 && (
              <p className="col-span-full text-center text-slate-500 py-12">
                ...
                <Link href="/contact" className="text-blue-600 font-semibold">contact us</Link>.
              </p>
            )}
            {!coursesLoading && displayCourses.map((course, i) => {
              const Icon = getCourseIcon(course.title, course.category);
              return (
                <motion.div key={course.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.1 }} className="group rounded-3xl p-[1px] bg-gradient-to-b from-slate-200 to-slate-100 hover:from-blue-600 hover:to-cyan-400 transition-all duration-500 overflow-hidden">
                  <div className="h-full bg-white rounded-[23px] p-8 flex flex-col">
                    <div className="w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center mb-6 text-slate-600 group-hover:text-white group-hover:bg-gradient-to-br group-hover:from-blue-600 group-hover:to-cyan-500 transition-all duration-300 shadow-sm">
                      <Icon size={28} />
                    </div>
                    <h3 className="text-2xl font-bold mb-3 text-slate-900">{course.title}</h3>
                    <p className="text-slate-600 mb-8 flex-grow line-clamp-3">{course.description}</p>
                    <Link href="/courses" className="w-full py-3 rounded-xl border border-slate-200 font-semibold text-slate-700 group-hover:bg-blue-600 group-hover:text-white group-hover:border-transparent transition-all duration-300 flex items-center justify-center">
                      Learn More
                    </Link>
                  </div>
                </motion.div>
      ...
      <section className="py-24 px-6 relative">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold mb-6 text-slate-900">Why Choose <span className="text-blue-600">Vivexa</span></h2>
            <p className="text-slate-600 max-w-2xl mx-auto text-lg">We provide an ecosystem designed for your rapid growth, combining theory with intensive practical application.</p>
          </div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((feature, i) => {
              ...
              return (
                <motion.div key={i} variants={fadeInUp} className="p-8 rounded-3xl bg-white border border-slate-200 hover:border-blue-500/50 transition-all duration-300 group hover:-translate-y-2 hover:shadow-xl hover:shadow-blue-500/5">
        ...
        <div className="container mx-auto max-w-7xl grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="p-10 rounded-[2.5rem] bg-white/60 border border-slate-200 backdrop-blur-2xl shadow-2xl hover:shadow-blue-500/10 transition-shadow group relative overflow-hidden">
              ...
              <h3 className="text-3xl font-black text-slate-900 mb-4">Powered By Vivexa Tech</h3>
              ...
              <Link href="/contact" className="inline-flex items-center justify-center w-full py-4 rounded-xl bg-slate-900 text-white font-bold hover:shadow-lg transition-all active:scale-95 group/btn">
      ...
      <section className="py-24 px-6 relative">
        <div className="container mx-auto max-w-6xl text-center">
          <h2 className="text-3xl md:text-5xl font-bold mb-16 text-slate-900">Why Students <span className="text-blue-600">Choose Vivexa</span></h2>
          
          <div className="grid md:grid-cols-3 gap-8">
            {TRUST_PILLARS.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            ...
            >
              <h2 className="text-3xl md:text-5xl font-black mb-6 text-slate-900">
                Verify Certificates <span className="text-blue-600">Online</span>
              </h2>
              <p className="text-lg text-slate-600 mb-10 leading-relaxed">
            ...
            >
              <div className="p-8 md:p-10 rounded-[2.5rem] bg-white border border-slate-200 shadow-2xl relative">
                ...
                <div className="flex items-center gap-4 mb-8 pb-8 border-b border-slate-100">
                  ...
                  <div>
                    <h4 className="text-xl font-bold text-slate-900">Certificate Search</h4>
                ...
                <div className="space-y-6">
                  ...
                  <div className="mt-8 p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-700">Secure verification portal</p>
                      <p className="text-xs text-slate-500 mt-0.5">Use your unique certificate ID to confirm authenticity</p>
                    </div>
                  </div>
                </div>
